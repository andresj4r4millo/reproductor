...
def abrir_archivo():
    ...
    pos = 0
    n = 0
    direcion = filedialog.askopenfilenames(
        initialdir="/",
...
tiempo_total.grid(row=0, column=7)
...
venta = Button(
    frame2, text="VENTA", bg="red", command=lambda: venta(cancion_actual, duracion)
)


venta.grid(column=3, row=2, pady=10)
no_venta = Button(
    frame2, text="NO VENTA", bg="red", command=lambda: no_venta(cancion_actual, duracion)
)
